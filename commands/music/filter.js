        ...
        try {
            ...
            if (!args[0])
                return message.channel.send(new MessageEmbed()
                    ...
                    .setDescription(`Filtros disponiveis: \`3d, bassboost, echo, karaoke, nightcore, vaporwave, clear, 8D, phaser, tremolo, vibrato, reverse, treble, normalizer, surrounding, pulsator, subboost, flanger, gate, haas, mcompand, cursed\``)
                    .setFooter("Para desativar algum filtro da fila apenas digite ele denovo")
                );
            const filters = args.join(', ');
            try {
                for(let i = 0; i < args.lenght; i++){
                    var filter = client.distube.setFilter(message, args[i]);
                }
                if (filter === null) return filters = "desligado";
                message.channel.send(new MessageEmbed()
                    .setColor(ee.yescolor)
                    .setTitle(`✅ SUCESSO | Filtro(s) da fila`)
                    .setDescription(`Filtro(s): \`${filters}\``)
                )
            } catch (e) {
                    message.channel.send(new MessageEmbed()
                    .setColor(ee.wrongcolor)
                    .setTitle(`❌ ERRO | Filtro Inválido`)
                    .setDescription(`\`${filters}\` não é um filtro válido`)
                )
                message.delete();
                return;
            }
